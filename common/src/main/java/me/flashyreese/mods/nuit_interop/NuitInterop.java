...
public class NuitInterop {
    ...
    private void convertNamespace(ResourceManagerHelper resourceManagerHelper, String skyParent, Pattern pattern) {
        ...
        if (NuitInteropConfig.INSTANCE.mode == NuitInteropMode.NATIVE) {
            if (!overworldLayers.isEmpty()) {
                ...
                SkyboxManager.getInstance().addSkybox(ResourceLocation.tryBuild("nuit_interop", String.format("native-%s-custom-sky-overworld", skyParent.split("/")[0])), skybox);
            }
            ...
            if (!endLayers.isEmpty()) {
                ...
                SkyboxManager.getInstance().addSkybox(ResourceLocation.tryBuild("nuit_interop", "native-optifine-custom-sky-end"), skybox);
            }
        }
    }
    ...
    private void convert(ResourceManagerHelper resourceManagerHelper, String skyParent, String skyName, ResourceLocation propertiesId, Properties properties, String world) {
        ...
        try {
            textureId = new ResourceLocation(namespace, path);
    ...
    private void generateSky(String dimension, String type) {
        ...
        ResourceLocation identifier = ResourceLocation.tryBuild("nuit_interop", type);
    ...
    private void generateOverworldDecorations() {
        ...
        ResourceLocation identifier = ResourceLocation.tryBuild("nuit_interop", "overworld-decorations");
        SkyboxManager.getInstance().addSkybox(identifier, json);
        this.convertedSkyMap.put(identifier, GSON.toJson(json));
    ...
    private void processProperties(JsonObject json, Properties properties, String skyName) {
        ...
        if (properties.containsKey("startFadeIn") && properties.containsKey("endFadeIn") && properties.containsKey("endFadeOut")) {
            ...
            JsonObject fadeKeyFrames = new JsonObject();
            fadeKeyFrames.addProperty(String.valueOf(Utils.normalizeTickTime(startFadeIn)), 0);
            fadeKeyFrames.addProperty(String.valueOf(Utils.normalizeTickTime(endFadeIn)), 1);
            fadeKeyFrames.addProperty(String.valueOf(Utils.normalizeTickTime(startFadeOut)), 1);
            fadeKeyFrames.addProperty(String.valueOf(Utils.normalizeTickTime(endFadeOut)), 0);

            fade.add("keyFrames", fadeKeyFrames);
        } else {
            fade.addProperty("alwaysOn", true);
        }
